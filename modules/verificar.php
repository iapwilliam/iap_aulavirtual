...
$token = $util->encrypt($_GET['token'], KEY_ENCRYPT);
...
if (in_array($token, $simple) || in_array($token, $doble)) {
    $alumno = $student->GetInfo("AND controlNumber = {$token}"); 
    $diploma['alumno'] = $alumno;
    if (in_array($token, $doble)) {
        $course->setCourseId(7);
        $dataCurso = $course->getCourse();
        $diploma['curso'] = $dataCurso['subject_name'];
        $course->setCourseId(8);
        $dataCurso = $course->getCourse();
        $diploma['curso'] .= " Y " . $dataCurso['subject_name'];
    } else {
        $course->setCourseId(7);
        $dataCurso = $course->getCourse();
        $diploma['curso'] = $dataCurso['subject_name'];
