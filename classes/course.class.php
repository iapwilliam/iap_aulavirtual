...
class Course extends Subject
{
	...
	{
		$sql = "SELECT major.name as major_name, subject.name as subject_name, course.courseId, `course`.`group`, subject.icon, subject.subjectId FROM course INNER JOIN subject ON subject.subjectId = course.subjectId INNER JOIN major ON major.majorId = subject.tipo WHERE 1 {$where}";
		$this->Util()->DB()->setQuery($sql);
		$result = $this->Util()->DB()->GetResult();
		return $result;
	}
	...
	public function dt_diplomas()
	{
		...
		$columns = array(
			...
			array('db'	=> '(SELECT GROUP_CONCAT(diplomas_cursos.course_id) FROM diplomas_cursos WHERE diplomas_cursos.diploma_id = diplomas.id)', 'dt'	=> 'cursos'),
			array(
				'db' => 'id',
				'dt' => 'acciones',
				'formatter' => function ($d, $row) {
	...
	{
		$sql = "INSERT INTO diplomas_cursos(diploma_id, course_id) VALUES($diploma, $curso)";
		$this->Util()->DB()->setQuery($sql);
		return $this->Util()->DB()->InsertData();
	}
